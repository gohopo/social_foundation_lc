...
public class MiPushService extends PushMessageReceiver {
    private static final AVLogger LOGGER = LogUtil.getLogger(MiPushService.class);
    ...
    private void updateAVInstallation(String miRegId) {
        if (!StringUtil.isEmpty(miRegId)) {
            ...
            if (!localProfile.equals(miDeviceProfile)) {
                installation.put(MIXPUSH_PROFILE, miDeviceProfile);
            }
